...
builds player distributions -- either fit from historical data, or
loaded directly from a custom field file if ``data.field_file`` is
set -- simulates one static season, one dynamic-weight season, and a
Monte Carlo batch of seasons, and writes all three results to the
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if settings.data.field_file:
            player_params = load_custom_field(settings.data.field_file)
        else:
